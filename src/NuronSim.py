...
# Set True to use recommended_trotter_steps(t) (src/optimal_trotter_steps.py,
# notes/optimal-trotter-steps.md) instead of a single fixed NumberOfTrotterSteps for
# every point in the sweep. D-15 found step count sets noise exposure independent of t
# while Trotter error grows with t, so a single fixed count can't be optimal everywhere
# in the sweep -- the schedule picks the (t-dependent) count that minimizes their sum.
# NumberOfTrotterSteps is still used as the fixed count when this is False.
...
# Only affects the noisy path (UseNoiseModel=True): sample_shots_with_postselection always
# computes both estimates regardless, so this is just which one becomes the plotted/saved
# "primary" result; both remain available in the saved .npz either way (occ/mag _post vs
...
# step schedule, post-selection -- are active), so sweeping configurations produces
# self-describing, non-clobbering output and an informative plot title.
...
# print("=================================")
# print("   BOSONIC DISPLACEMENT GATE ")
# print("=================================")
# bosonic_gate = BosonicDisplacementGate(D_list[0], NumberOfFockStates, Time / NumberOfTrotterSteps)
# bosonic_qubits = cirq.LineQubit.range(NumberOfFockStates)
# decomposed_bosonic = cirq.Circuit(cirq.decompose(bosonic_gate(*bosonic_qubits), keep=lambda op: isinstance(op.gate, (cirq.ISwapPowGate))))
# print(decomposed_bosonic)
# print("\n")

# print("=================================")
# print("   SPIN INTERACTIONS GATE ")
# print("=================================")
# spin_gate = SpinInteractionsGate(spin_interaction_coefficient, NumberOfBosonicModes, Time / NumberOfTrotterSteps)
# spin_qubits = cirq.LineQubit.range(NumberOfBosonicModes)
# decomposed_spin = cirq.Circuit(cirq.decompose(spin_gate(*spin_qubits), keep=lambda op: isinstance(op.gate, (cirq.ISwapPowGate))))
# print(decomposed_spin)
# print("\n")

# print("=================================")
# print("   TROTTER STEP CIRCUIT ")
# print("=================================")
# circuit = NetworkOfNeuronsTrotterStep(D_list, spin_interaction_coefficient, NumberOfFockStates, NumberOfBosonicModes, Time / NumberOfTrotterSteps, SpinBosonInteractionCoefficent)
# print(circuit)
...
for time_idx, t in enumerate(time_data):
    ...
    # 1-3. Vacuum state prep + steps_t repeats of the dt-step
...
# =====================================================================
# Remove any Cirq scatter artists left over from a previous run of this cell
# (e.g. after changing NumberOfTrotterSteps and re-running), so datapoints
# from old runs don't keep piling up on top of each other.
...
# qutip_fig.suptitle(
#     f"QuTiP (lines) vs Cirq Trotterized simulation on Willow with noise (points) \n Number of trotter steps = {NumberOfTrotterSteps}, Number of Fock states = {NumberOfFockStates} \n Number of Bosonic modes = {NumberOfBosonicModes}, D_j = {[round(d, 3) for d in D_list]} \n Spin interaction coeficent = {spin_interaction_coefficient} \n Spin-Boson interaction coeficent = {SpinBosonInteractionCoefficent:.3g}",
#     fontweight='bold',
#     fontsize=10,
#     y=0.98,
# )
